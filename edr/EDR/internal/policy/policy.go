...
const SchemaVersion = "v0.7"

// AcceptedSchemaVersions lists schema versions this engine can load.
// v0.1 policies load with zero-valued new fields (backward compatible).
var AcceptedSchemaVersions = []string{"v0.1", "v0.5", "v0.7", "v0.7-exercise"}
...
type Match struct {
	...
	// v0.5 internal network detection matchers
	ParentProcessName  string `json:"parent_process_name,omitempty"`     // parent process comm name
	ProcessUID         string `json:"process_uid,omitempty"`             // effective UID string match
	FileContent        string `json:"file_content,omitempty"`            // substring match on file content (first 4KiB)
	FilePathSuffix     string `json:"file_path_suffix,omitempty"`        // file path suffix match
	ConnectionFreqMin  int    `json:"connection_freq_min,omitempty"`     // minimum connections per interval
	BeaconIntervalMin  int    `json:"beacon_interval_min_sec,omitempty"` // C2 beacon min interval (seconds)
	BeaconIntervalMax  int    `json:"beacon_interval_max_sec,omitempty"` // C2 beacon max interval (seconds)
	ContainerID        string `json:"container_id,omitempty"`            // /proc/pid/cgroup container ID
}
...
type Subject struct {
	...
	ParentName  string `json:"parent_name,omitempty"`  // parent process comm name
	EUID        string `json:"euid,omitempty"`         // effective UID string
	ContainerID string `json:"container_id,omitempty"` // container ID from /proc/pid/cgroup
}
...
type Object struct {
	...
	// v0.5 enrichment fields
	FileContent     string `json:"file_content,omitempty"`     // file content for content-based matching
	ConnectionCount int    `json:"connection_count,omitempty"` // connection count from ConnTracker
}
...
func (p Policy) Validate() (warnings []string, err error) {
	if !acceptedSchemaVersion(p.SchemaVersion) {
		return nil, fmt.Errorf("unsupported schema_version %q (accepted: %v)", p.SchemaVersion, AcceptedSchemaVersions)
	}
	...
	for i, r := range p.Rules {
		...
		seen[r.ID] = true
		if !oneOf(r.Category, "process", "file", "network", "rootkit") {
		...
		if !oneOf(r.Action, "none", "kill", "quarantine", "fix_permissions", "nft_block", "log", "fanotify_deny", "kill_tree", "network_isolate", "process_suspend") {
		...
		case "block":
			if !oneOf(r.Action, "kill", "fix_permissions", "nft_block", "quarantine", "fanotify_deny", "kill_tree", "network_isolate", "process_suspend") {
				return nil, fmt.Errorf("rule %s: decision=block requires action in {kill, fix_permissions, nft_block, quarantine, fanotify_deny, kill_tree, network_isolate, process_suspend}, got %q", r.ID, r.Action)
			}
		}
...
func (pa ProcessAccess) Validate() error {
	...
	if !oneOf(pa.Action, "none", "kill", "kill_tree") {
		return fmt.Errorf("process_access.action must be none, kill, or kill_tree")
	}
...
func (p Policy) EvaluateProcessAccess(subj Subject) (Rule, bool) {
	...
	// DEFAULT-DENY MODE: When enforce mode is active and a whitelist is
	// configured, any process that matches NEITHER the whitelist NOR the
	// blacklist is blocked by default. This creates a process lockdown:
	// only processes explicitly listed in the whitelist are allowed to
	// run; everything else (including interactive shells like bash/sh)
	// receives a "block" decision with the configured action (typically
	// "kill").
	//
	// When whitelist is empty and enforce mode is active, only processes
	// matching the blacklist are blocked (default-allow behavior).
	// Ensure all legitimate system processes are included in the
	// whitelist when using it in enforce mode, or SSH sessions and
	// system management will break.
...
func (p Policy) evaluateAll(now time.Time, subj Subject, obj Object) []Rule {
	...
	var out []Rule
	category := inferCategory(subj, obj)
...
func inferCategory(subj Subject, obj Object) string {
	switch {
	case obj.FilePath != "" || obj.FileOp != "":
		return "file"
	case obj.RemoteAddr != "" || obj.LocalPort != 0 || obj.Protocol != "":
		return "network"
	case subj.ProcessName != "" || subj.ProcessPath != "" || subj.Cmdline != "" ||
		subj.User != "" || subj.Environ != "" || subj.MapsLibs != "" || subj.PtraceSelfCheck:
		return "process"
	default:
		return ""
	}
...
func matches(m Match, subj Subject, obj Object) bool {
	...
	// v0.5 internal network detection matchers
	if m.ParentProcessName != "" && !strings.EqualFold(m.ParentProcessName, subj.ParentName) {
		return false
	}
	if m.ProcessUID != "" && m.ProcessUID != subj.EUID {
		return false
	}
	if m.FileContent != "" && !strings.Contains(obj.FileContent, m.FileContent) {
		return false
	}
	if m.FilePathSuffix != "" && !strings.HasSuffix(obj.FilePath, m.FilePathSuffix) {
		return false
	}
	if m.ConnectionFreqMin != 0 && obj.ConnectionCount < m.ConnectionFreqMin {
		return false
	}
	if m.ContainerID != "" && m.ContainerID != subj.ContainerID {
		return false
	}
	// Beacon matching is handled at the agent level (ConnTracker.FindBeacons),
	// not in the per-rule matches() function. The BeaconIntervalMin/Max fields
	// are used by the agent to configure beacon detection, not for direct matching.
...
func acceptedSchemaVersion(v string) bool {
	for _, a := range AcceptedSchemaVersions {
		if v == a {
			return true
		}
	}
