...
func TestEvaluateAllFiltersByInferredCategory(t *testing.T) {
	p := Policy{SchemaVersion: SchemaVersion, Rules: []Rule{
		{ID: "rootkit-any", Category: "rootkit", Severity: "critical", Decision: "block", Action: "network_isolate", Match: Match{}},
		{ID: "file-open", Category: "file", Severity: "high", Decision: "block", Action: "fanotify_deny", Match: Match{FilePathPrefix: "/opt/edr/", FileOp: "open"}},
		{ID: "proc-ptrace", Category: "process", Severity: "high", Decision: "alert", Action: "none", Match: Match{PtraceSelfCheck: boolPtr(true)}},
	}}
	if _, err := p.Validate(); err != nil {
		t.Fatal(err)
	}

	fileMatches := p.EvaluateAll(time.Now(), Subject{ProcessName: "cat"}, Object{FilePath: "/opt/edr/edr-agent", FileOp: "open"})
	if len(fileMatches) != 1 || fileMatches[0].ID != "file-open" {
		t.Fatalf("expected only file-open for file event, got %+v", fileMatches)
	}

	procMatches := p.EvaluateAll(time.Now(), Subject{ProcessName: "probe", PtraceSelfCheck: true}, Object{})
	if len(procMatches) != 1 || procMatches[0].ID != "proc-ptrace" {
		t.Fatalf("expected only proc-ptrace for process event, got %+v", procMatches)
	}
}

func boolPtr(v bool) *bool { return &v }

func TestEvaluateAllDoesNotInferRootkitFromOrdinarySignals(t *testing.T) {
	p := Policy{SchemaVersion: SchemaVersion, Rules: []Rule{
		{ID: "rk", Category: "rootkit", Severity: "critical", Decision: "block", Action: "network_isolate", Match: Match{}},
	}}
	if _, err := p.Validate(); err != nil {
		t.Fatal(err)
	}
	if matches := p.EvaluateAll(time.Now(), Subject{ProcessName: "bash"}, Object{}); len(matches) != 0 {
		t.Fatalf("unexpected rootkit match for ordinary process subject: %+v", matches)
	}
	if matches := p.EvaluateAll(time.Now(), Subject{}, Object{FilePath: "/etc/edr/policy.json", FileOp: "write"}); len(matches) != 0 {
		t.Fatalf("unexpected rootkit match for ordinary file object: %+v", matches)
	}
}
