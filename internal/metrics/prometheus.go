...
func WritePrometheus(w io.Writer, src MetricsSource) {
	...
	// Rootkit detection
	writeCounter(w, "edr_rootkit_checks_total", "Total rootkit cross-source checks", toUint64(m["rootkit_checks_total"]))
	writeCounter(w, "edr_rootkit_findings_total", "Total rootkit findings", toUint64(m["rootkit_findings_total"]))
}
