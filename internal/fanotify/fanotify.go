...
func (p *Provider) mark(root string) error {
	...
	err := filepath.WalkDir(root, func(path string, d fs.DirEntry, err error) error {
		...
		if err := p.markOne(path); err != nil {
			// ENOSPC / EPERM on individual subdirs are non-fatal.
			return nil
		}
		marked++
		if marked >= 4096 {
...
func (p *Provider) handleEvent(meta fanotifyEventMetadata) {
	...
	if isPerm {
		info := p.resolveAccessInfo(meta, path)
		...
		if err := writeResponse(p.fd, fanotifyResponse{Fd: meta.Fd, Response: respVal}); err != nil {
			...
			// response. Log the error; the kernel will implicitly
			// deny or the process will hang. There's nothing we can
			// do to recover, but at least we have a record.
			fmt.Fprintf(os.Stderr, "fanotify: %v (pid=%d path=%s)\n", err, meta.Pid, path)
		}
...
func (p *Provider) resolveAccessInfo(meta fanotifyEventMetadata, path string) AccessInfo {
	return AccessInfo{
		PID:     meta.Pid,
