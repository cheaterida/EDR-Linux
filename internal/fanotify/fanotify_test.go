...
func TestEmitAuditUsesDecisionAsAction(t *testing.T) {
	logger := &fakeLogger{}
	p := &Provider{logger: logger}
	p.emitAudit(fanotifyEventMetadata{Pid: int32(os.Getpid()), Mask: FAN_OPEN_PERM}, "/tmp/test", true, "rule-1")
	p.emitAudit(fanotifyEventMetadata{Pid: int32(os.Getpid()), Mask: FAN_OPEN_PERM}, "/tmp/test", false, "rule-2")
	...
	if logger.events[0].Action != "allow" || logger.events[0].Decision != "allow" {
		t.Fatalf("allow audit mismatch: %+v", logger.events[0])
	}
	if logger.events[1].Action != "block" || logger.events[1].Decision != "block" {
		t.Fatalf("block audit mismatch: %+v", logger.events[1])
	}
}

func TestShouldBypassPermissionCheckAllowsCriticalSystemPaths(t *testing.T) {
	if !shouldBypassPermissionCheck("cat", "/usr/bin/cat", "/etc/ssh/sshd_config") {
		t.Fatal("expected ssh config path to bypass permission check")
	}
}

func TestShouldBypassPermissionCheckProtectsEDRPathsFromGenericTools(t *testing.T) {
	if shouldBypassPermissionCheck("cat", "/usr/bin/cat", "/opt/edr/edr-agent") {
		t.Fatal("generic tools must not bypass EDR binary protection")
	}
	if shouldBypassPermissionCheck("sed", "/usr/bin/sed", "/etc/edr/policy.json") {
		t.Fatal("generic tools must not bypass EDR config protection")
	}
}

func TestShouldBypassPermissionCheckAllowsEDRSelfAccess(t *testing.T) {
	if !shouldBypassPermissionCheck("edr-agent", "/opt/edr/edr-agent", "/opt/edr/edr-agent") {
		t.Fatal("edr-agent should be allowed to access its own binary")
	}
	if !shouldBypassPermissionCheck("edrctl", "/opt/edr/edrctl", "/etc/edr/policy.json") {
		t.Fatal("edrctl should be allowed to access EDR config")
	}
}
