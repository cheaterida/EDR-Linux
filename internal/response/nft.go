...
// Default DROP policy, only allows loopback and established/related.
func (p NFTProvider) ApplyIsolate() Result {
	table := sanitizeNFTIdent(valueOr(p.Table, "edr"), "edr")
	isolateChain := "isolate"

	cmds := [][]string{
		// Create isolate table and chain with DROP policy
		{"nft", "add", "table", "inet", table},
		{"nft", "add", "chain", "inet", table, isolateChain, "{", "type", "filter", "hook", "output", "priority", "0", ";", "policy", "drop", ";", "}"},
		// Allow loopback
		{"nft", "add", "rule", "inet", table, isolateChain, "oifname", "lo", "accept"},
		// Allow established/related connections (for agent communication)
		{"nft", "add", "rule", "inet", table, isolateChain, "ct", "state", "established,related", "accept"},
		// Allow DNS (for name resolution)
		{"nft", "add", "rule", "inet", table, isolateChain, "udp", "dport", "53", "accept"},
		{"nft", "add", "rule", "inet", table, isolateChain, "tcp", "dport", "53", "accept"},
	}
	...
	if p.DryRun || !p.Enabled {
		...
		return Result{Action: "network_isolate", Success: true, Detail: "nft dry-run: " + strings.Join(desc, " && ")}
	}
