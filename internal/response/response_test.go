...
func TestKillRejectsMissingIdentityWithoutTransientFlag(t *testing.T) {
	res := SoftResponder{}.Apply(ActionRequest{Action: "kill", PID: os.Getpid()})
	if res.Success || res.Detail == "" {
		t.Fatalf("expected missing identity rejection, got %#v", res)
	}
}

func TestKillAllowsTransientPidfdFallbackForCurrentProcess(t *testing.T) {
	res := SoftResponder{DryRun: true}.Apply(ActionRequest{Action: "kill", PID: os.Getpid(), TransientOK: true})
	if !res.Success {
		t.Fatalf("expected dry-run transient kill success, got %#v", res)
	}
}
