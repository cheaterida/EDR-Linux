...
type ActionRequest struct {
	...
	TransientOK bool   `json:"transient_ok,omitempty"`
}
...
func (r SoftResponder) Apply(req ActionRequest) Result {
	...
	case "kill":
		...
		if req.TransientOK && req.ProcessPath == "" && req.StartTicks == "" {
			if err := PidfdKill(req.PID); err == nil {
				return Result{Action: req.Action, Success: true, Detail: fmt.Sprintf("killed pid %d via transient pidfd", req.PID)}
			} else if err != errPidfdNotSupported {
				return Result{Action: req.Action, Success: false, Detail: err.Error()}
			}
			return Result{Action: req.Action, Success: false, Detail: "identity unavailable and pidfd not supported"}
		}
		if !sameProcess(req) {
