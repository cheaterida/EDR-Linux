...
type WebhookConfig struct {
	...
	SharedSecret string           `json:"shared_secret,omitempty"` // HMAC shared secret for authenticated peer ingest
}
...
func (d *WebhookDispatcher) sendOne(c *webhookClient, ev WebhookEvent) {
	...
	if c.cfg.SharedSecret != "" {
		...
		mac.Write(body)
		req.Header.Set("X-EDR-Ingest-Signature", "sha256="+hex.EncodeToString(mac.Sum(nil)))
	}
