...
func main() {
	...
	// not load BPF, fanotify, or any policy — it only checks heartbeats.
	if *guardianMode {
		if flag.NArg() < 2 {
			fmt.Fprintf(os.Stderr, "guardian: missing agent-pid and run-dir arguments\n")
			os.Exit(1)
		}
		agentPID, _ := strconv.Atoi(flag.Arg(0))
		runDir := flag.Arg(1)
		guardian(agentPID, runDir)
		return
	}
	...
	if mf, ok := loader.(bpf.MapFiller); ok {
		...
		if !*once {
			...
			// v0.16: bpf_guard — Phase 4 enforcement: blocks BPF_MAP_UPDATE_ELEM
			// from non-edr-agent processes at ring0, preventing agent_pid map
			// tampering and BPF program detachment.
			if err := mf.SetBpfGuard(true); err != nil {
				fmt.Fprintf(os.Stderr, "edr-agent: set bpf_guard BPF map: %v\n", err)
			}
		}
	...
	adminKey, _ := adminauth.LoadKey(cfg.AdminAuth.AdminKeyPath)
	...
	if cfg.Fanotify.Enabled && len(cfg.Fanotify.Paths) > 0 {
		...
		if err != nil {
			...
			// Register inode-based self-protection for EDR files.
			// These are matched by device+inode, immune to symlink
			// and bind-mount bypasses.
			agentPID := os.Getpid()
			protectFiles := []struct{ path, desc string }{
				{"/opt/edr/edr-agent", "edr-agent-binary"},
				{"/opt/edr/edrctl", "edrctl-binary"},
				{"/etc/edr", "edr-config-dir"},
	...
	// Dual-guardian: fork a lightweight watchdog child process that
	// monitors agent heartbeat and re-execs the agent if it stalls.
	// The agent writes its heartbeat every tick cycle; the guardian
	// checks every 3s. If guardian dies, agent re-forks it.
	runDir := "/run/edr"
	hbPath := filepath.Join(runDir, "agent.hb")
	gHbPath := filepath.Join(runDir, "guardian.hb")
	_ = os.MkdirAll(runDir, 0o755)
	startGuardian := func() {
		agentPID := os.Getpid()
		os.WriteFile(filepath.Join(runDir, "agent.pid"), []byte(fmt.Sprintf("%d\n", agentPID)), 0o600)
		// ForkExec re-execs the current binary as a child process.
		// The guardian runs in a separate process with its own Go runtime.
		...
		attr := &syscall.ProcAttr{
			Dir:   "/",
			Env:   os.Environ(),
			Files: []uintptr{0, 1, 2}, // inherit stdio for logging
			Sys:   &syscall.SysProcAttr{Setsid: true},
		}
		pid, err := syscall.ForkExec(exe, []string{exe, "--guardian", fmt.Sprintf("%d", agentPID), runDir}, attr)
		if err == nil && pid > 0 {
			os.WriteFile(filepath.Join(runDir, "guardian.pid"), []byte(fmt.Sprintf("%d\n", pid)), 0o600)
		}
	}
	startGuardian()

	lastGuardianCheck := time.Now()
	for {
		select {
		case sig := <-sigCh:
		...
		case <-ticker.C:
			...
			os.WriteFile(hbPath, []byte(fmt.Sprintf("%d\n", time.Now().Unix())), 0o600)

			// Check guardian health every 10s. If guardian heartbeat
			// is stale (>20s), re-fork.
			if time.Since(lastGuardianCheck) > 10*time.Second {
				lastGuardianCheck = time.Now()
				raw, err := os.ReadFile(gHbPath)
				if err != nil {
					startGuardian()
				} else {
					ts, err := strconv.ParseInt(strings.TrimSpace(string(raw)), 10, 64)
					if err != nil || time.Now().Unix()-ts > 20 {
						startGuardian()
					}
				}
			}
...
func (h *fanotifyHandler) HandleFileAccess(info fanotify.AccessInfo) (allow bool, ruleID string) {
	...
	resp, audits := policy.AggregatedDecision(matches)

	// Collect all matched rule IDs for audit logging and metrics.
	var ids []string
	if resp != nil {
		ids = append(ids, resp.ID)
	}
	for i := range audits {
		ids = append(ids, audits[i].ID)
	}
	ruleID = strings.Join(ids, ",")

	// Record metrics for every matched rule so rule_hits reflect
	// fanotify-evaluated matches, not just the procfs path.
	for i := range matches {
		h.agent.RecordRuleHit(matches[i].ID)
	}
...
func execSelf() {
	exe, err := os.Executable()
	if err != nil {
		fmt.Fprintf(os.Stderr, "edr-agent: cannot find self: %v\n", err)
		os.Exit(1)
	}
	if err := syscall.Exec(exe, os.Args, os.Environ()); err != nil {
		fmt.Fprintf(os.Stderr, "edr-agent: re-exec failed: %v\n", err)
		os.Exit(1)
	}
}

// guardian runs as a forked child process. Its sole responsibility is
// to monitor the main agent's heartbeat and re-exec the agent if it
// stops updating. The guardian itself writes its own heartbeat so the
// agent can detect guardian death and re-fork.
func guardian(agentPID int, runDir string) {
	// Detach from controlling terminal.
	syscall.Setsid()

	hbPath := filepath.Join(runDir, "agent.hb")
	gHbPath := filepath.Join(runDir, "guardian.hb")
	os.WriteFile(gHbPath, []byte(fmt.Sprintf("%d\n", os.Getpid())), 0o600)

	ticker := time.NewTicker(3 * time.Second)
	defer ticker.Stop()

	for range ticker.C {
		// Write guardian heartbeat.
		os.WriteFile(gHbPath, []byte(fmt.Sprintf("%d\n", time.Now().Unix())), 0o600)
		...
		if time.Now().Unix()-ts > 15 {
			// Agent appears dead — re-exec it.
			fmt.Fprintf(os.Stderr, "edr-guardian: agent heartbeat lost, restarting agent\n")
			exe, err := os.Executable()
			if err != nil {
				os.Exit(1)
			}
