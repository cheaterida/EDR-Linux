...
> **当前版本：v0.4++** | 目标平台：Ubuntu 22.04 | 语言：Go 1.22 + eBPF (C)

一个面向教学和安全研究的 Linux EDR (Endpoint Detection and Response) 系统。从 ring3 用户态采集到 ring0 内核态探针，覆盖 **采集 → 策略匹配 → 响应阻断 → 审计取证** 完整闭环。
...
| `action` | string | `kill` / `fix_permissions` / `fanotify_deny` / `nft_block` / `none` |
...
项目预置了 19 条规则，覆盖常见攻击场景：

| 规则 ID | 检测目标 | 响应 |
|---------|----------|------|
| P001 | curl 下载执行 | kill |
| P003 | bash 反弹 shell | kill |
| P003b | Python 反弹 shell | kill |
| P005 | /tmp 目录执行 | kill |
| N001 | 可疑端口监听 (4444) | nft_block |
| ATT001 | ptrace 反调试 | alert |
| ATT002 | LD_PRELOAD 注入 | kill |
| ATT003 | Frida 插桩检测 | kill |
| ATT004 | 修改 /etc/ld.so.preload | fix_permissions |
| SELF001 | 访问 agent 二进制 | fanotify_deny |
...
│   └── response/                       响应层
│       ├── response.go                kill/fix_permissions
│       └── nft.go                     nftables 规则
│
├── configs/                            配置文件
